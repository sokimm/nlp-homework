...
def chunks(l, n, states, vocab, transition, emission):
    chunk_size = len(l) // n
...
def main():
    ...
    if len(text_file_lines) <= NUM_PROCESSES * PROCESSOR_RATIO:
        results = viterbi(text_file_lines, states, vocab, transition, emission)

    # Otherwise divide workload amongst process threads
    else:
        slices = chunks(text_file_lines, NUM_PROCESSES, states, vocab, transition, emission)
