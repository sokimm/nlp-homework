...
##########
My viterbi.py for triagram and smoothing
...
# HMM_FILE = 'train_task2_smoothing.hmm'
# TEXT_FILE = 'ptb.23.txt'
# OUTPUT_FILE = 'ptb.23.tgs'


TRI_TRANSITION_TAG = "tri_trans"
BI_TRANSITION_TAG = "bi_trans"
UNI_PROB_TAG = "uni_prob"
EMISSION_TAG = "emit"
...
uni_prob = defaultdict(lambda: 1.0)
...
def viterbi(lines):
    ...
    for (index, line) in enumerate(lines):
        ...
        # V[x][y][z] where x is the index of the word in the line
        #   and y is a pre-state in the set of states and z is a curr-state
        ...
        #   and z is the previous of previous state with the highest probability
        ...
        for i in range(len(w)):
            ...
            for q in states:
                ...
                # Specifically, qq -> q
                for qq in states:    
                    backoff = 0 # if tri_transition failed, do backoff
                    for qqq in states:
                        # If this transition is impossible for this state, move on
                        
                        # smoothing
                        #transition_prob = lambda_1*tri_transition[qqq][qq][q] +\
                        #    lambda_2*bi_transition[qq][q]+lambda_3*uni_prob[q]
                        
                        transition_prob = tri_transition[qqq][qq][q]
        ...
        for qq in states:
            for q in states:
                #transition_prob = lambda_1*tri_transition[qq][q][FINAL_STATE]+\
                #    lambda_2*bi_transition[q][FINAL_STATE]+lambda_3*uni_prob[q]
                transition_prob = tri_transition[qq][q][FINAL_STATE]
        ...
        if best_final_state:
            ...
            for i in range(len(w) - 1, -1, -1):
                ...
                best_final_state_1 = best_prefinal_state
                best_prefinal_state = back[i][best_prefinal_state][best_final_state]
                best_final_state = best_final_state_1
...
def main():
    ...
    with open(HMM_FILE, "r") as f:
        for line in f:
            ...
            elif line[0] == BI_TRANSITION_TAG:
                (qq, q, trans_prob) = line[1:4]
                bi_transition[qq][q] = math.log(float(trans_prob))
                states.add(qq)
                states.add(q)
            
            elif line[0] == UNI_PROB_TAG:
                (q, prob) = line[1:3]
                uni_prob[q] = math.log(float(trans_prob))
                states.add(q)
